module ActiveRecord
  module Validations
    module ClassMethods
      TAG_RE = /<.[^<>]*?>/.freeze
      PUNCTUATION_RE = %r{[–—.(),;:!?%#{$‘’“”}/"_+=\/-]*}.freeze
      WHITESPACE_RE = /(&nbsp;|&#160;|[ ])+/i.freeze
      ...
      def validates_word_count(*args)
        ...
        if configuration[:in]&.is_a?(Range)
          minimum = configuration[:in].min
          maximum = configuration[:in].max
        else
          minimum = configuration[:minimum].to_i if configuration[:minimum]&.is_a?(Integer)

          maximum = configuration[:maximum].to_i if configuration[:maximum]&.is_a?(Integer)
        end

        raise 'Must supply a maximum or minimum word count' if maximum.nil? && minimum.nil?

        raise 'Maximum word count must be greater than 0' if !maximum.nil? && (maximum == 0)

        if !maximum.nil? && !minimum.nil?
          raise "Minimum word count (#{minimum}) cannot be greater than maximum (#{maximum})" if minimum > maximum
        end
